  ...
  return (
    <div className={cn("group/message flex gap-3 px-4 py-4", isUser ? "justify-end" : "justify-start")}>
      ...
      <div className={cn("flex max-w-[92%] min-w-0 flex-col gap-2 sm:max-w-[78%]", isUser ? "items-end" : "items-start")}>
        <div
          className={cn(
            "text-[11px] font-medium uppercase tracking-widest opacity-70",
            isUser ? "text-muted-foreground/80" : "text-primary/70",
          )}
        >
        ...
        {isUser ? (
          <div className="rounded-3xl rounded-tr-sm bg-gradient-to-br from-primary to-primary/90 px-5 py-3 text-[15px] leading-relaxed text-primary-foreground shadow-md">
            <div className="whitespace-pre-wrap break-words">{message.content}</div>
          </div>
        ) : (
          <>
            {showLoading ? (
              <div className="rounded-3xl rounded-tl-sm border border-white/10 bg-background/60 px-5 py-3.5 text-sm text-muted-foreground shadow-sm backdrop-blur-xl">
                <span className="inline-flex items-center gap-2">
                  ...
                  <span className="ml-1 animate-pulse">Đang phân tích tài liệu...</span>
                </span>
              </div>
            ) : null}
            ...
            {message.content ? (
              <div
                className={cn(
                  "rounded-3xl rounded-tl-sm border border-white/10 bg-background/80 px-5 py-3.5 text-[15px] leading-relaxed text-foreground shadow-sm backdrop-blur-xl",
                  isStreaming && "streaming-active border-primary/20 bg-background/90 shadow-primary/5",
                )}
              >
            ...
            {!!citations.length && !isStreaming ? (
              <div className="w-full rounded-2xl border border-white/10 bg-background/60 p-4 shadow-sm backdrop-blur-md transition-all hover:bg-background/80">
