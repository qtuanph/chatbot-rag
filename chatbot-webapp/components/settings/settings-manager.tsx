...
export function SettingsManager({ initialBilling = null }: SettingsManagerProps) {
  const { data: session } = useSession();
  const isPlatformAdmin = session?.role === "platform_admin";

  const [billingData, setBillingData] = useState<BillingSettings | null>(initialBilling);
  const [billingLoading, setBillingLoading] = useState(isPlatformAdmin && !initialBilling);
  const [billingSaving, setBillingSaving] = useState(false);

  useEffect(() => {
    if (isPlatformAdmin && !billingData) {
