"""RAGAS Evaluation - Measure RAG quality metrics using OpenAILike (9Router)."""
...
class RagasEvaluator:
    ...
    async def evaluate(
        ...
        if not answer or not contexts:
            return RagasaMetrics(
                faithfulness=0.0, answer_relevancy=0.0, context_relevancy=0.0, answer_similarity=0.0, overall_score=0.0
            )
    ...
    async def _get_score(self, prompt: str) -> float:
        try:
            provider = self.ai_provider or Settings.llm
            response = await provider.acomplete(prompt)
            text = response.text.strip() if response and response.text else "0.5"
            import re
