...
def build_vector_store(
    *,
    ...
    When ``native_sparse=True`` the caller is responsible for providing
    sparse vectors via direct Qdrant API — no BM25 fallback is configured.
    """
    return QdrantVectorStore(
        ...
        fastembed_sparse_model=None if native_sparse else ("Qdrant/bm25" if enable_hybrid else None),
        payload_indexes=get_payload_indexes(),
        flat_metadata=False,
    )


def get_section_vector_store(native_sparse: bool = False) -> QdrantVectorStore:
...
def get_chunk_vector_store(native_sparse: bool = False) -> QdrantVectorStore:
    return build_vector_store(
