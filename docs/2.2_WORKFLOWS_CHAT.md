# 2.2 — Chat → Retrieve → Generate → Response

## Overview

```mermaid
sequenceDiagram
    participant Browser
    participant API
    participant Cache as SemanticCache (Redis Vector)
    participant ChatSvc
    participant Retriever as LlamaIndex (VectorStoreIndex)
    participant Qdrant
    participant AIEmbed as ai-embedding (TEI)
    participant AIRerank as reranker (TEI / NVIDIA)
    participant LLM as OpenAILike (9Router)

    Browser->>API: POST /chat/stream (SSE)
    API->>ChatSvc: prepare_chat()
    ChatSvc->>ChatSvc: normalize / refine / expand query
    ChatSvc->>Retriever: retrieve_context()

    Retriever->>Cache: semantic_cache.get(query_vector)
    alt Cache HIT (Similarity > 92%)
        Cache-->>Retriever: cached RagContext
    else Cache MISS
        Retriever->>AIEmbed: embed_query()
        AIEmbed-->>Retriever: query vector
        Retriever->>Qdrant: hybrid search (dense + native BM25 RRF)
        Qdrant-->>Retriever: candidate chunks
        Retriever->>AIRerank: rerank (TEI or NVIDIA)
        AIRerank-->>Retriever: scored chunks
        Retriever->>Cache: semantic_cache.set(result)
    end

    Retriever-->>ChatSvc: RagContext
    ChatSvc->>LLM: OpenAILike.astream_chat() via 9Router
    LLM-->>Browser: SSE Stream (chunks)
```

## Retrieval Flow

The retrieval pipeline (`app/modules/chat/retrieval/pipeline.py`) uses LlamaIndex primitives:

1. **Embed query** → `TextEmbeddingsInference` (TEI ai-embedding, 768-dim)
2. **Hybrid search** → `VectorStoreIndex.as_retriever(mode="hybrid")` — dense + Qdrant native BM25 with RRF fusion (top_k=20)
3. **Rerank** → `TEIRerankerPostprocessor` or `NvidiaRerankerPostprocessor` (switchable via `RERANKER_BACKEND`)
4. **Deduplicate** → by node_id across multiple query variants
5. **Assemble context** → full section text + metadata → `RagContext`

## Cache Architecture (3 layers)

| Layer | Key | TTL | Hit → |
|-------|-----|-----|-------|
| LLM Response Cache | `hash(normalized_query)` | 4h | Return immediately (bypasses LLM) |
| Semantic Cache | `vector(query_embedding)` | 24h | Return RAG context |
| Query Embedding Cache | `hash(normalized_query)` | 4h | Skip embedding |

## Reranking (Switchable Backend)

After hybrid search, the system reranks candidate chunks via `get_reranker()`:

| Backend | When | Details |
|---------|------|---------|
| `TEI` (default) | `RERANKER_BACKEND=tei` | Local TEI container, `POST /rerank`, gte-multilingual-reranker-base |
| `NVIDIA` | `RERANKER_BACKEND=nvidia` | NVIDIA NIM API, `nvidia/llama-nemotron-rerank-vl-1b-v2` |

## Query Handling

### Normalization

All cache layers use normalized queries:
- Lowercase
- Strip whitespace
- Collapse multiple spaces
- Remove stopwords (Vietnamese/ERP boilerplate)

Example: "Xin chào, cho tôi biết SEO là gì?" → "seo là gì"

### Refinement (Optional)

If `RETRIEVAL_QUERY_REFINEMENT_ENABLED=true`, a small LLM call rewrites the query to be more search-friendly (timeout: `RETRIEVAL_QUERY_REFINEMENT_TIMEOUT`).

### Expansion (Optional)

If `RETRIEVAL_QUERY_EXPANSION_ENABLED=true`, the LLM generates up to 3 query variants for broader retrieval coverage.

## Chat Invariants

| Rule | Requirement |
|------|-------------|
| **SemanticCache** | Redis Vector Search checks for similarity > 92% (COSINE distance < 0.08) |
| **Exact Cache** | Redis exact match check on raw query text for sub-millisecond response |
| **Binary Serialization** | Chat history stored using **MessagePack** for extreme speed and low RAM |
| 2-stage retrieval | Hybrid search (dense + BM25 RRF) → rerank |
| Rate limiting | **Sliding Window (Redis Lua)** — 30 req/min per user |
| History limit | `AI_MAX_HISTORY_MESSAGES=6` — last N messages sent to LLM |

## History Limiting

Conversation history sent to LLM is limited to `AI_MAX_HISTORY_MESSAGES` (default 6) messages. No context compaction — simpler, faster, no extra API calls needed.
