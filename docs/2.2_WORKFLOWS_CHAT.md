...
For short queries (<5 words), the system generates a hypothetical answer document using the auxiliary model, embeds it, and adds the resulting vector to the query ensemble.

**When**: Only for short queries (<5 words, the ones that benefit most from HyDE). Skipped for multi-query expansions with >2 variants. Timeout: 1.5s. Conservative prompt to minimize hallucination risk.

**Benefit**: +10-20% precision for short/ambiguous queries; noise reduction on longer queries.
...
| 5-stage retrieval | HyDE (<5 words only, conservative prompt) → Hybrid search (dense + BM25 RRF) → section grouping (≥0.25) + Rerank → Neighbor Expansion → confidence scoring → context assembly |
