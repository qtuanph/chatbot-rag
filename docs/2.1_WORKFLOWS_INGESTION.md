...
sequenceDiagram
    ...
    LiPipeline->>Qdrant: QdrantVectorStore.add() — auto-embed via TEI + native BM25
...
| Hybrid indexing | Both dense (768-dim) and native BM25 sparse vectors stored in Qdrant |
...
| 9 | Embed & Store | `QdrantVectorStore.add()` — auto-embeds via `Settings.embed_model` (TEI, 768-dim, batch size 32), stores dense + native BM25 sparse vectors |
...
### Hybrid Search: Dense + Native BM25

| Component | Details |
|-----------|---------|
| Dense model | gte-multilingual-base (768-dim, cosine) |
| Sparse model | **Qdrant native BM25** — no custom encoder needed |
