...
sequenceDiagram
    ...
    participant Parser as Docling + EasyOCR
    ...
    Parser-->>Worker: Docling Documents
...
| 4 | EasyOCR | force_full_page_ocr=True — Engine: EasyOCR via RapidOCR ONNX |
| 4b | DOCX → PDF | DOCX converted to PDF via LibreOffice headless before OCR (flattens Word numbering) |
...
| 7 | Section extraction | Docling items → sections with exact page spans → breadcrumbs |
...
|----------|------|--------|
| EasyOCR (`force_full_page_ocr=True`) | All PDFs + images | `converter` + RapidOcrOptions vi+en |
| DOCX → PDF → OCR | All DOCX files | LibreOffice headless → EasyOCR (flattens Word numbering) |
| Classic parser | XLSX, TXT, Markdown | No OCR — native text extraction |
...
|----------------|--------|
| EasyOCR (mandatory) | `app/modules/documents/ingestion/ingestion_service.py` |
