"""
Audit Stream Worker: Consumes security events from Redis and persists to DB.
"""
...
def process_audit_stream(batch_size: int = 100):
    """
    Consumes a batch of audit events from Redis and writes them to the database.
    This task should be scheduled periodically (e.g., every 5-10 seconds) via Celery Beat.
    """
    import asyncio
    
    # We use a helper to run the async logic
    async def _process():
        try:
            # XREAD from stream
