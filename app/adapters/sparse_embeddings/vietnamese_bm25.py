...
_VIETNAMESE_STOP_WORDS: frozenset[str] = frozenset(
    {
        "và", "của", "là", "có", "không", "được", "trong", "với", "cho", "này", "đó", "những", "từ", "để", "một",
        "các", "về", "nếu", "thì", "đã", "sẽ", "đang", "vẫn", "phải", "nên", "bị", "bởi", "vì", "nhưng", "hay",
        "hoặc", "thành", "ra", "vào", "lên", "xuống", "lại", "mà", "cũng", "đều", "nào", "đâu", "sao", "thế",
        "khi", "nơi", "vậy", "gì", "ai", "bao_giờ", "tại", "theo", "cùng", "nữa", "rất", "hơn", "nhất", "mỗi",
        "tất_cả", "riêng", "khác", "như", "thường", "lúc", "ngay", "chỉ", "từng", "vừa", "mới", "luôn", "tự",
        "tới", "đến", "hầu_hết", "chưa", "chẳng", "biết", "thấy", "làm", "đi", "cái", "con", "chiếc", "sự",
        "việc", "nhà", "người", "anh", "chị", "em", "ông", "bà", "bạn", "họ", "mình", "ta", "tôi", "chúng_ta",
        "chúng_tôi", "chúng_họ", "đây", "kia", "ấy", "nay", "vừa_qua", "trước", "sau", "vừa_mới", "vừa_xong",
    }
)
...
class VietnameseBM25Encoder:
    ...
    def tokenize(self, text: str) -> list[str]:
        ...
        for t in re.findall(r"\w+", segmented.lower()):
            if (
                t not in _VIETNAMESE_STOP_WORDS 
                and len(t) > 1 
                and not t.isnumeric()
            ):
                tokens.append(t)
    ...
    def encode(self, text: str) -> dict[str, Any]:
        ...
        return {
            "indices": [idx for idx, _ in raw_indices],
            "values": [float(count) for _, count in raw_indices]
        }
